...
| 简历定制 | 在“可视化编排 / YAML 源码”间切换；选择全部模板（ModernCV/Jake/Jake 原版/Calm/VS Code），拖拽内容、章节以及可分别选择的中文/英文字体组件，或直接编辑全部 YAML 文件。未发布的模板、内容、字体与画布顺序草稿按数据仓和简历类型自动保存在本机，返回页面或刷新后继续上次状态；“预览”只生成临时产物，“保存发布正式版”才把 `fonts.cjk/latin` 等配方保存并进入版本时间轴 |
