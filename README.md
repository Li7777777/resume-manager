...
| PDF 预览 | 一键组合 + 本地构建 + 浏览器内渲染 PDF（react-pdf，缩放/翻页/下载）；GitHub 编译开启时可从 CI 同步 Action 产出的 PDF 预览（无需本地环境） |
| 历史版本 | **GitHub 提交时间轴（左）+ 对应版本的 PDF（右，react-pdf 渲染）+ 该提交的 YAML 数据快照**——每个历史版本与其数据一一匹配 |
