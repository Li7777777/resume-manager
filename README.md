...
| 信息管理 | 分类管理（工作/教育/项目/技能/证书/兴趣/基础信息），基础信息支持上传 JPEG/PNG 证件照并自动加入各简历模板；标签筛选与搜索，可视化增删改，成就点逐条打标签 |
...
- 私有仓只保留简历内容、证件照 `assets/profile-photo.*`、组稿所需的 `id/tags`、组稿规则与 CI，不保存管理端设置字段；
