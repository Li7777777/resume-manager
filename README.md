...
npm run smoke:photo  # 临时数据仓中验收一寸证件照裁剪
...
| 信息管理 | 分类管理（工作/教育/项目/技能/证书/兴趣/基础信息），基础信息支持选择最大 20 MB 的 JPEG/PNG 证件照，在浏览器中按标准一寸（25 × 35 mm / 295 × 413 px / 300 DPI）拖动、缩放和裁剪后自动加入各简历模板；标签筛选与搜索，可视化增删改，成就点逐条打标签 |
| 简历类型 | 轻量管理多个简历类型；每个类型对应 `resume/<type>` Git 分支，支持创建、切换、改名和删除 |
| 简历定制 | 在“可视化编排 / YAML 源码”间切换；选择全部模板（ModernCV/Jake/Jake 原版/Calm/VS Code），拖拽内容、章节以及可分别选择的中文/英文字体组件（列表从当前系统实时扫描并支持搜索），或直接编辑全部 YAML 文件。未发布的模板、内容、字体与画布顺序草稿按数据仓和简历类型自动保存在本机，返回页面或刷新后继续上次状态；“预览”只生成临时产物，“保存发布正式版”才把 `fonts.cjk/latin` 等配方保存并进入版本时间轴 |
...
- 私有仓只保留简历内容、裁剪后的证件照 `assets/profile-photo.*`、组稿所需的 `id/tags`、组稿规则与 CI，不保存管理端设置字段；证件照原图只在浏览器中参与裁剪，不上传、不落盘；
