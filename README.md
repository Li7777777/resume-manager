...
| 简历定制 | 在“可视化编排 / YAML 源码”间切换；选择全部官方模板（ModernCV/Jake/Calm/VS Code），拖拽内容与章节或直接编辑全部 YAML 文件。未发布的可视化草稿按数据仓和简历类型自动保存在本机，返回页面或刷新后继续上次状态；“预览”只生成临时产物，“保存发布正式版”才保存配方并进入版本时间轴 |
